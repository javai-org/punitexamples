package org.javai.punit.examples.scripts
...
private fun extractMethodName(title: String, codeBlock: String, type: BlockType): String {
    return when (type) {
        BlockType.TEST_CONFIGURATION -> {
            // Title may be "TEST CONFIGURATION FOR: methodName" or just "TEST CONFIGURATION".
            // When the title contains "FOR:", extract the method name from it.
            // Otherwise, extract the method name from the first non-blank content line
            // of the code block (after the banner line).
            if ("FOR:" in title) {
                ...
                val contentLine = lines
                    ...
                    ?.trim() ?: error("Cannot find method name in TEST CONFIGURATION block")
...
private fun assemble(summaryIndex: BlockIndex, verboseIndex: BlockIndex): String {
    ...
    for (section in sections()) {
        // Check whether all required code blocks for this section are available.
        // If any are missing (e.g. covariate tests that need a baseline not yet
        // generated), skip the section with a placeholder.
        val missingBlocks = section.parts.filterIsInstance<Part.CodeBlock>().filter { part ->
            val index = if (part.level == "SUMMARY") summaryIndex else verboseIndex
            index.getOrNull(part.method, part.type) == null
        }
        ...
        if (missingBlocks.isNotEmpty()) {
            ...
            sb.appendLine()
            sb.appendLine("> **Not yet available.** This section requires blocks that were not " +
                    "produced by the current test run. Missing: " +
                    missingBlocks.joinToString { "${it.method} (${it.type}, ${it.level})" })
            continue
        }
